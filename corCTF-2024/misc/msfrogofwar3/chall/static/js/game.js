    ...
    if (data.status !== "running") {
        setTimeout(() => {
            ...
            if (data.status === "draw") {
                ...
                $("#turn").text(`Checkmate - ${state.status === "win" ? 'You win' : 'You lose!'} (${state.turn_counter})`);
            }
        }, 500);
    }
});
...
const board = Chessboard('chessboard', {
    ...
    onDrop: (source, target) => {
        ...
        // check if move exists, or move exists with promotion
        if (!state.moves.includes(action) && !["q", "n", "b", "r"].some(p => state.moves.includes(action + p)))
            return "snapback";
